...
<style>
  ...
  body, .stApp {background-color: #F4F3EF !important;}
</style>
...
STATUS_MAP = {"Work InProgress":"Work In Progress","Work in progress":"Work In Progress","TODO":"To Do"}
...
CARD = "background:#fff;border-radius:12px;border:1px solid #e8e6e0;padding:18px 20px;"
...
with col_hdr:
    st.markdown(f"""
    <div style="background:#0F1F3D;color:white;border-radius:12px;padding:20px 28px;
                display:flex;justify-content:space-between;align-items:center;">
      <div>
        <div style="font-size:19px;font-weight:600;margin-bottom:4px">
...
# ── KPIs ─────────────────────────────────────────────────
st.markdown("<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:8px'>Key metrics at a glance</p>", unsafe_allow_html=True)
...
for col, lbl, val, sub in [
    ...
    with col:
        ...
        <div style="{CARD}">
          ...
          <div style="font-size:36px;font-weight:700;color:#1a1a18;line-height:1.1">{val}</div>
...
# ── Insights ─────────────────────────────────────────────
st.markdown("<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:8px'>Key insights</p>", unsafe_allow_html=True)
...
for col, bc, tc, title, body in [
    (i1, "#EF4444", "#991B1B", "Backlog trend",   f"Net {ns}{net} tickets this week — active count is {len(active)}. {trend}"),
    (i2, "#F59E0B", "#92400E", "Aging tickets",   f"Oldest open ticket is {oldest_age} days old ({oldest_key}). Review tickets aged over 60 days."),
    (i3, "#3B82F6", "#1E40AF", "Production bugs", bug_msg),
]:
    with col:
        st.markdown(f"""
        <div style="background:white;border-left:4px solid {bc};border-top:1px solid #e8e6e0;
                    border-right:1px solid #e8e6e0;border-bottom:1px solid #e8e6e0;
                    border-radius:0 10px 10px 0;padding:14px 16px;min-height:90px">
          <div style="font-weight:700;font-size:13px;color:{tc};margin-bottom:5px">{title}</div>
...
with c1:
    st.markdown(f"<div style='{CARD}padding:16px 18px;'>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:4px'>Active ticket status</p>", unsafe_allow_html=True)
    sc = {}
    ...
    if sc:
        ...
        fig.update_layout(**CHART_BG,
            legend=dict(orientation="h", xanchor="center", x=0.5, yanchor="top", y=-0.05, font=dict(size=10)))
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar":False})
    st.markdown("</div>", unsafe_allow_html=True)

with c2:
    st.markdown(f"<div style='{CARD}padding:16px 18px;'>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:4px'>Active by request type</p>", unsafe_allow_html=True)
    rc = {}
    for i in active:
        v=get_rt(i)
...
with c3:
    st.markdown(f"<div style='{CARD}padding:16px 18px;'>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:4px'>Resolution reasons — this week ({len(res_week)} tickets)</p>", unsafe_allow_html=True)
    rb_wk = resb(res_week)
    if rb_wk:
...
with c4:
    st.markdown(f"<div style='{CARD}padding:16px 18px;'>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:4px'>Resolution reasons — this month ({len(res_mon)} tickets)</p>", unsafe_allow_html=True)
    rb_mo = resb(res_mon)
    if rb_mo:
...
st.markdown("<hr style='border:none;border-top:1px solid #e8e6e0;margin:16px 0'>", unsafe_allow_html=True)
...
with t1:
    st.markdown(f"<div style='{CARD}padding:16px 18px;'>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:8px'>Aging backlog — oldest open tickets</p>", unsafe_allow_html=True)
    if aged:
        df_age = pd.DataFrame([{
            "Ticket": i["key"],
...
with t2:
    st.markdown(f"<div style='{CARD}padding:16px 18px;'>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#888;margin-bottom:8px'>Open production bugs ({len(bugs)})</p>", unsafe_allow_html=True)
    if bugs:
...
st.markdown(f"<p style='text-align:center;font-size:11px;color:#aaa;margin-top:24px;padding-top:16px;border-top:1px solid #e8e6e0'>Support Operations Live Dashboard · groundgamehealth.atlassian.net · {now_str}</p>", unsafe_allow_html=True)